...
/// {@category Amazing}
class MapsHelper {
  static Set<DocumentSnapshot> currentDeadZones = new Set<DocumentSnapshot>();
  ...
  static void setUserId() async {
    ...
    if (email != null) {
      ...
      if (querySnapshot.docs.length > 1) {
        print("Multiple people with the same email.");
      } else {
  ...
  static Future<DocumentSnapshot> getEmployeePinDoc(
    ...
    var pinsEmployeeNumbers =
        new List<int>.filled(pinsReference.docs.length, 0);
    var pinsDocuments =
        new List<DocumentSnapshot>.filled(pinsReference.docs.length, null);
  ...
  static Future<bool> createVideoMarker(String urld, bool isVideo, int pintype,
    ...
    if (ConstantsClass.businessAccount) {
      businessPost ap = new businessPost(
        datePosted: new DateTime.now().toIso8601String(),
        url: urld,
        isVideo: isVideo,
        likes: 0,
        likesUids: "",
      ...
      for (DocumentSnapshot deadzone in currentDeadZones) {
        if (pointInCircle(
          ...
          print("User is in the deadzone so video will not be uploaded.");
          return false;
        }
      }

      LatLng where = new LatLng(pos.latitude, pos.longitude);
      String myVideoToken = await PushNotificationService.getDeviceToken();

      if (MapsPage.answering) {
        AnswerPin ap = new AnswerPin(
          email: user.email,
        ...
        firestore.runTransaction((Transaction tx) async {
          DatabaseService()
              ...
              .catchError((e) {})
              .whenComplete(() {
            // This is where we would most likely refresh/delete the request pin.
...
Future<int> sendFollowersNotifications(Map tokenMap) async {
  ...
  for (var v in tokens) {
    print("Sending notification to follower.");
